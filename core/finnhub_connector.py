...
def get_session_key() -> str:
    ...
    return normalize_api_key(st.session_state.get(KEY_STATE, ""))
...
def connect(key: str) -> Dict[str, Any]:
    ...
    if result.get("ok"):
        ...
        st.session_state[ERROR_STATE] = _redact(result.get("message", "Connection failed"), clean)
    return result


def test_connection(candidate_key: str = "") -> Dict[str, Any]:
    """Test the typed candidate or the currently connected session key.

    When a candidate is supplied and succeeds, it becomes the connected key so
    users can press either Connect or Test after pasting a token.
    """
    initialize_finnhub_state()
    ...
    if result.get("ok"):
        ...
        st.session_state[ERROR_STATE] = _redact(result.get("message"), key)
    return result
...
def render_finnhub_connector(*, location: str = "sidebar") -> None:
    """Render the canonical connector inside the existing Research > NLP area."""
    initialize_finnhub_state()
    ...
    st.caption("Session-only key. It is never written to project files, logs, copies, exports, or the prediction ledger.")
    if str(location).lower() == "settings":
        # A normal password input can be difficult to long-press/paste into on
        # iPhone Safari and embedded mobile webviews.  Settings therefore owns
        # one larger paste-friendly field.  Other locations keep the masked
        # desktop input for backward compatibility.
        candidate = st.text_area(
            "Finnhub API key — mobile paste box",
            key=widget_key,
            height=74,
            placeholder="Long press here, paste the token from your Finnhub dashboard, then press Connect",
            help="The token stays in this Streamlit session only.",
        )
    else:
        candidate = st.text_input("Finnhub API key", type="password", key=widget_key, placeholder="Paste the token from your Finnhub dashboard")
    typed_key = normalize_api_key(candidate)
    stored_key = get_session_key()
    c1, c2, c3 = st.columns(3)
    if c1.button("Connect", key=f"finnhub_connect_{location}", use_container_width=True, disabled=not bool(typed_key)):
        with st.spinner("Validating Finnhub with a news-safe endpoint…"):
            connect(typed_key)
        _safe_rerun()
    if c2.button(
        "Test",
        key=f"finnhub_test_{location}",
        use_container_width=True,
        disabled=not bool(typed_key or stored_key),
    ):
        with st.spinner("Testing Finnhub authentication and news access…"):
            test_connection(typed_key)
        _safe_rerun()
    if c3.button(
        "Disconnect",
        key=f"finnhub_disconnect_{location}",
        use_container_width=True,
        disabled=not bool(status.get("connected") or stored_key),
    ):
        disconnect()
        st.session_state["finnhub_widget_generation"] = generation + 1
        _safe_rerun()

    status = connection_status()
    m1, m2 = st.columns(2)
    m1.metric("Connection", "CONNECTED" if status["connected"] else "DISCONNECTED")
    ...
    if status.get("error"):
        st.warning(_redact(status["error"], stored_key or typed_key))
