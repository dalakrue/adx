...
def _render_lunch(ns: dict, subpage: str) -> None:
    if not subpage:
        ...
        # One authoritative Lunch surface: exactly four closed-first core fields.
        _safe_component("Lunch Four Core Fields", render_lunch_quick_decision)
        return
...
def _render_mobile_api_key_center() -> None:
    """Paste-friendly secret fields for phones; existing connector logic is unchanged."""
    st.markdown("""
    <style>
    ...
    st.markdown("### 🔑 Mobile API Key Paste Center")
    st.caption("Long-press inside the larger boxes to paste on iPhone/mobile. Keys stay in Streamlit session state and are not written into project files.")

    twelve_generation = int(st.session_state.get("settings_mobile_twelve_generation_20260619", 0) or 0)
    with st.expander("Open / Close — Twelve Data API Key", expanded=True):
        current = str(st.session_state.get("twelve_api_key", "") or "")
        value = st.text_area(
            "Twelve Data API key — mobile paste box",
            value=current,
            key=f"settings_mobile_twelve_api_key_paste_20260619_{twelve_generation}",
            height=76,
            placeholder="Long press here, paste the Twelve Data API key, then press Save",
        )
        c1, c2 = st.columns(2)
        if c1.button("Save Twelve Key", key="settings_mobile_save_twelve_20260619", use_container_width=True):
            st.session_state["twelve_api_key"] = str(value or "").strip()
            st.success("Twelve Data key saved for this session. Open the market connector below and press Connect / Refresh.")
        if c2.button("Clear Twelve Key", key="settings_mobile_clear_twelve_20260619", use_container_width=True):
            st.session_state["twelve_api_key"] = ""
            st.session_state["settings_mobile_twelve_generation_20260619"] = twelve_generation + 1
            st.success("Twelve Data key cleared.")
            _safe_rerun()
    ...
    with st.expander("Open / Close — NLP / AI Assistant API Key", expanded=False):
        current = str(st.session_state.get("nlp_api_key", "") or "")
        value = st.text_area(
            "NLP API key — mobile paste box",
            value=current,
            key=f"settings_mobile_nlp_api_key_paste_20260619_{nlp_generation}",
            height=76,
        ...
        c1, c2 = st.columns(2)
        if c1.button("Save NLP Key", key="settings_mobile_save_nlp_20260619", use_container_width=True):
            ...
            st.success("NLP API settings saved for this session.")
...
def _render_settings() -> None:
    ...
    # Restored Settings controls.  They reuse the existing connector, timer and
    # authentication state; no market calculation or trading rule is changed.
