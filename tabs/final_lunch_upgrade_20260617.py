...
# Compatibility note for restoration tests: the delegated four-field renderer uses
...
def render_lunch_quick_decision() -> None:
    """Render the authoritative four-field Lunch surface only."""
    if not _lunch_route_active():
        return
    started = time.perf_counter()
    from ui.lunch_four_core_fields_20260619 import render_lunch_four_core_fields
    render_lunch_four_core_fields(state=st.session_state)
