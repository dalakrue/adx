...
def _render_quick_actions() -> None:
    ...
    if c1.button("▶ Run All", key="main_menu_run_calculation_20260615", use_container_width=True):
        try:
            ...
            st.session_state["active_page"] = "Lunch" if bool((status.get("canonical") or {}).get("ok")) else "Settings"
            st.session_state["active_subpage"] = ""
            st.session_state["tab_choice"] = st.session_state["active_page"]
        except Exception as exc:
            try:
                from core.operational_sync_20260618 import record_operational_error
                record_operational_error(st.session_state, "Main Drawer Run All", exc, stage="calculation")
            except Exception:
                pass
