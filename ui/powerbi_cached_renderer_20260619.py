...
def _render_cached_chart(
    ...
    if not main_view.empty:
        if main_view["upper"].notna().any() and main_view["lower"].notna().any():
            fig.add_trace(go.Scatter(x=main_view["time"], y=main_view["upper"], mode="lines", line={"width": 1, "color": "rgba(90,140,255,0.55)"}, name="P90 / upper band"))
            fig.add_trace(go.Scatter(x=main_view["time"], y=main_view["lower"], mode="lines", line={"width": 1, "color": "rgba(90,140,255,0.55)"}, fill="tonexty", fillcolor="rgba(90,140,255,0.12)", name="P10 / lower band"))
        if main_view["p75"].notna().any() and main_view["p25"].notna().any():
            fig.add_trace(go.Scatter(x=main_view["time"], y=main_view["p75"], mode="lines", line={"width": 1, "color": "rgba(130,180,255,0.55)"}, name="P75"))
            fig.add_trace(go.Scatter(x=main_view["time"], y=main_view["p25"], mode="lines", line={"width": 1, "color": "rgba(130,180,255,0.55)"}, fill="tonexty", fillcolor="rgba(130,180,255,0.18)", name="P25"))
        fig.add_trace(go.Scatter(x=main_view["time"], y=main_view["main"], mode="lines+markers", line={"width": 3, "color": "#f4f7ff"}, marker={"size": 5}, name="Central forecast path"))
    if show_paths:
    ...
    if not main_view.empty:
        for hour in (1, 2, 3, 6):
            if len(main_view) >= hour:
                ...
                fig.add_trace(go.Scatter(
                    x=[row["time"]], y=[row["main"]], mode="markers+text",
