...
def _render_connector(*, key_prefix: str = "main_drawer", show_secret_inputs: bool = True) -> None:
    ...
    c1, c2 = st.columns(2)
    with c1:
        if st.button("✅ Connect / Refresh", key=f"{key_prefix}_connect_api_v1", use_container_width=True):
            _connect_now("Main-page drawer connect", quick=False)
            request_close_native_sidebar()
    with c2:
        if st.button("⛔ Disconnect", key=f"{key_prefix}_disconnect_api_v1", use_container_width=True):
